...
export const CCAgents: React.FC<CCAgentsProps> = ({ onBack, className }) => {
  ...
  const loadAgents = async () => {
    try {
      ...
      setError(t("messages.failedToLoadAgents"));
      setToast({ message: t("messages.failedToLoadAgents"), type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const confirmDeleteAgent = async () => {
    ...
    try {
      ...
      setToast({ message: t("messages.agentDeletedSuccess"), type: "success" });
      await loadAgents();
      await loadRuns(); // Reload runs as they might be affected
    } catch (err) {
      console.error("Failed to delete agent:", err);
      setToast({ message: t("messages.failedToDeleteAgent"), type: "error" });
    } finally {
      setIsDeleting(false);
  ...
  const handleAgentCreated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t("messages.agentCreatedSuccess"), type: "success" });
  };

  const handleAgentUpdated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t("messages.agentUpdatedSuccess"), type: "success" });
  };
  ...
  const handleExportAgent = async (agent: Agent) => {
    try {
      ...
      setToast({ message: t("messages.agentExportedSuccess", { name: agent.name }), type: "success" });
    } catch (err) {
      console.error("Failed to export agent:", err);
      setToast({ message: t("messages.failedToExportAgent"), type: "error" });
    }
  };
  ...
  const handleImportAgent = async () => {
    try {
      ...
      setToast({ message: t("messages.agentImportedSuccess"), type: "success" });
      await loadAgents();
    } catch (err) {
      console.error("Failed to import agent:", err);
      const errorMessage = err instanceof Error ? err.message : t("messages.failedToImportAgent");
    ...
    <div className={cn("flex flex-col h-full bg-background", className)}>
                            ...
                            <Card className="h-full hover:shadow-lg transition-shadow">
                              <CardContent className="p-6 flex flex-col items-center text-center">
                                ...
                                <p className="text-xs text-muted-foreground">
                                  {t("agents.createdLabel")}{new Date(agent.created_at).toLocaleDateString()}
                                </p>
                              </CardContent>
      ...
      <GitHubAgentBrowser
        ...
        onImportSuccess={async () => {
          ...
          await loadAgents();
          setToast({ message: t("messages.agentImportedFromGitHubSuccess"), type: "success" });
        }}
      />
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("agents.deleteAgentConfirmation", { name: agentToDelete?.name })} 
