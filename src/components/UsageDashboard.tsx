...
export const UsageDashboard: React.FC<UsageDashboardProps> = ({ onBack }) => {
  ...
  const loadUsageStats = async () => {
    try {
      ...
      setError(t("usageDashboard.failedToLoad"));
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="flex space-x-1">
              {(["all", "30d", "7d"] as const).map((range) => (
                <Button
                  ...
                  className="text-xs"
                >
                  {t(`usageDashboard.dateRanges.${range}`)}
                </Button>
              ))}
            </div>
          </div>
          ...
          >
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <TabsList className="grid w-full grid-cols-5">
                <TabsTrigger value="overview">{t("usageDashboard.tabs.overview")}</TabsTrigger>
                <TabsTrigger value="models">{t("usageDashboard.tabs.models")}</TabsTrigger>
                <TabsTrigger value="projects">{t("usageDashboard.tabs.projects")}</TabsTrigger>
                <TabsTrigger value="sessions">{t("usageDashboard.tabs.sessions")}</TabsTrigger>
                <TabsTrigger value="timeline">{t("usageDashboard.tabs.timeline")}</TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="models">
                <Card className="p-6">
                  <h3 className="text-sm font-semibold mb-4">{t("usageDashboard.usageByModel")}</h3>
              ...
              <TabsContent value="projects">
                <Card className="p-6">
                  <h3 className="text-sm font-semibold mb-4">{t("usageDashboard.usageByProject")}</h3>
                  <div className="space-y-3">
                    {stats.by_project.map((project) => (
                      <div key={project.project_path} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {formatCurrency(project.total_cost / project.session_count)}/{t("usageDashboard.session")}
                          </p>
                        </div>
                      </div>
                    ))}
              ...
              <TabsContent value="sessions">
                  <Card className="p-6">
                      <h3 className="text-sm font-semibold mb-4">{t("usageDashboard.usageBySession")}</h3>
                      <div className="space-y-3">
                              ...
                              <div key={day.date} className="flex-1 h-full flex flex-col items-center justify-end group relative">
                                ...
                                <div className="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10">
                                  <div className="bg-background border border-border rounded-lg shadow-lg p-3 whitespace-nowrap">
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                      {day.models_used.length} {t("usageDashboard.model")}{day.models_used.length !== 1 ? 's' : ''}
                                    </p>
                                  </div>
